...
import { listConversations } from '../../session/api';
import type { ConversationSummary, Workspace } from '../../session/types';
...
interface WorkspaceConversationSummary extends ConversationSummary {
  ...
  useEffect(() => {
    ...
    void Promise.all(
      workspaces.map(async (workspace) =>
        (await listConversations(workspace.id)).map((conversation) => ({
          ...conversation,
      ...
      .then((items) => {
        if (!disposed) setConversations(items.flat());
      })
      .catch((caught) => {
    ...
    return [...conversations]
      .filter((conversation) => {
        ...
        return (
          ...
          (!query ||
            (conversation.title ?? 'New conversation').toLocaleLowerCase().includes(query) ||
            ...
            visible.map((conversation, index) => {
              ...
              return (
                <ButtonBase
                  ...
                  role="listitem"
                  aria-label={`Resume ${conversation.title ?? 'New conversation'} in ${workspaceName}`}
