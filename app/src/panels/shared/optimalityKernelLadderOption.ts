...
export function optimalityKernelLadderOption(
  ...
  return {
    ...
    series: identities.map((identity) => ({
      ...
      // Keep the other rungs readable while the tooltip identifies one exact
      // kernel segment. Series focus made every unrelated segment fade to 25%,
      // so moving the pointer across a stacked bar caused the whole ladder to
      // flash between muted states.
