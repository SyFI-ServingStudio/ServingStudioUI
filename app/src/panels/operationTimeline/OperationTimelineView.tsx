...
export type OperationTimelineState =
  | { readonly status: 'idle'; readonly worker: null }
  | { readonly status: 'loading'; readonly worker: { readonly key: string } }
  | {
      readonly status: 'ready';
      readonly worker: { readonly key: string };
      readonly viewport: OperationViewportState;
      readonly operations: readonly OperationSummary[];
      readonly selected: OperationSummary | null;
      shift(direction: OperationBufferDirection): void;
      navigate(operationDelta: number): void;
    }
  | {
      readonly status:
        'empty' | 'unavailable' | 'not_generated' | 'failed' | 'incompatible' | 'error';
      readonly worker: { readonly key: string } | null;
      readonly reason: string;
    };

export type OperationTimelineSeekState =
  | { readonly status: 'idle'; readonly atMs: null }
  | { readonly status: 'loading'; readonly atMs: number }
  | { readonly status: 'ready'; readonly atMs: number }
  | { readonly status: 'error'; readonly atMs: number; readonly reason: string };

export interface OperationTimelineViewProps {
  readonly state: OperationTimelineState;
  readonly seek: OperationTimelineSeekState;
...
const workerOperationLabel = (operation: OperationSummary): string =>
  `${operation.section} · layer ${operation.layer}`;
...
export default function OperationTimelineView({
  state,
  seek,
  selectOperation,
}: OperationTimelineViewProps) {
