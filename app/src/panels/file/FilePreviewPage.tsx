...
} from '../../session/workspaceFiles';
import type { FileRef } from '../../location';
...
import { fileName, pathAncestors } from '../filePath';
...
export default function FilePreviewPage({
  ...
  fileRef: FileRef;
  onOpenFile: (file: FileRef) => void;
  renderMarkdown: (text: string, workspaceId: string) => React.ReactNode;
}) {
  const { workspace: workspaceId, path, line } = fileRef;
      ...
      <SurfaceCard sx={{ p: 1.4 }}>
        <Stack direction="row" alignItems="center" sx={{ gap: 1.1, flexWrap: 'wrap' }}>
          ...
          <Box sx={{ minWidth: 0, flex: '1 1 200px' }}>
            ...
            <Breadcrumbs workspaceId={workspaceId} path={path} onOpenFile={onOpenFile} />
          </Box>
      ...
      <SurfaceCard accent={tokens.sub2} sx={{ minWidth: 0, overflow: 'hidden' }}>
        {failure ? (
          ...
          entries === null ? (
            ...
            <DirectoryView workspaceId={workspaceId} entries={entries} onOpenFile={onOpenFile} />
          )
          ...
          structured.kind === 'json' ? (
            ...
            <Box sx={{ p: 1.6, minWidth: 0, overflow: 'auto' }}>
              {renderMarkdown(displayText, workspaceId)}
            </Box>
          )
        ) : (
