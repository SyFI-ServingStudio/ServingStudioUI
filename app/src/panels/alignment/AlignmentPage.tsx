...
import { formatLocation, segmentOf, selectSegment } from '../../location';
 ...
 */
function queryOf<T>(result: ArtifactResult<T> | undefined) {
  return {
    data: result?.status === 'ready' ? result.value : undefined,
    isPending: result === undefined || result.status === 'pending',
    isError: result !== undefined && result.status !== 'ready' && result.status !== 'pending',
    error:
      result !== undefined && 'reason' in result
        ? new Error(result.reason ?? 'Alignment artifact unavailable.')
        : null,
  };
}
...
export function AlignmentPage({ location, navigate }: PanelProps) {
  ...
  return (
    <AlignmentContent
      location={location}
      navigate={navigate}
      result={{ ...location.ref, kind: 'alignment' as const }}
    />
  );
}
...
function AlignmentContent({
  ...
  readonly result: PanelProps['location']['ref'] & { readonly kind: 'alignment' };
}) {
  const descriptor = queryOf(useArtifact(alignmentDescriptorRef(result)));
  ...
  useEffect(() => {
    ...
    if (timeline.data !== undefined) {
      const initial = initialAlignmentIterationId(
        timeline.data.iterations,
        boardExampleIterationId,
      );
      if (initial !== null) {
        navigate(
          { ...location, focus: selectSegment(location.focus, { at: 'iteration', id: initial }) },
          'replace',
        );
      }
      return;
    ...
    if (fallback !== undefined && fallback !== null) {
      navigate(
        { ...location, focus: selectSegment(location.focus, { at: 'iteration', id: fallback }) },
        'replace',
      );
    }
  }, [
  ...
  const timelineDetailRefs: AlignmentTimelineIterationRef[] =
    hasDetail('timeline') && selectedIterationId !== null
      ? [alignmentTimelineIterationRef(result, selectedIterationId)]
      : [];
  const breakdownRefs: AlignmentBreakdownRef[] =
    hasDetail('iteration') && selectedIterationId !== null
      ? [alignmentBreakdownRef(result, selectedIterationId)]
      : [];
  const iteration = queryOf(useArtifacts(timelineDetailRefs)[0]);
  const breakdown = queryOf(useArtifacts(breakdownRefs)[0]);
  const selectIteration = (iterationId: number) =>
    navigate(
      { ...location, focus: selectSegment(location.focus, { at: 'iteration', id: iterationId }) },
      'replace',
    );
                    ...
                    descriptor.data.prediction === null
                      ...
                      : {
                          href: formatLocation({
                            ...
                            ref: {
                              kind: 'prediction',
                              id: descriptor.data.prediction.predictionId,
                              workspace: location.ref.workspace,
                            },
