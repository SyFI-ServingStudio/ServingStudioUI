...
function finiteDerived(value: number, path: string): number {
...
function validateScaleProducts(node: RawCostNode, multiplier: number, path: string): void {
  ...
  const nextMultiplier =
    node.kind === 'scale' ? finiteDerived(multiplier * node.n, `${path}.n`) : multiplier;
...
export function parseRawCostNode(input: unknown): RawCostNode {
  ...
  const root = immutableRawNode(result.data);
  validateRawCostTreeSemantics(root);
  return root;
}
