...
describe('offline Analyzer resources', () => {
  ...
  it('resolves only descriptor-declared measurement plots against Analyzer', () => {
    const descriptor = parseKernelMeasurementDescriptor(
      ...
      new URL('http://analyzer.test/api/analyzer/v1/'),
    );
    // A plot is named, not addressed: the URL below is derived from the
    // measurement id and the plot name alone.
    expect(descriptor.plotUrls).toEqual([
      'http://analyzer.test/api/analyzer/v1/kernel-measurements/km_one/plots/runtime_trend.png',
    ]);
  });
