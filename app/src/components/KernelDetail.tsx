...
function Item({ k, v, big, teal }: { k: string; v: string; big?: boolean; teal?: boolean }) {
  return (
    <Box sx={{ p: '12px 18px', borderTop: `1px solid ${tokens.hair}`, borderRight: `1px solid ${tokens.hair}`, '&:last-of-type': { borderRight: 'none' } }}>
      ...
      <Typography sx={{ fontFamily: big ? tokens.serif : tokens.mono, fontSize: big ? 22 : 12.5, fontWeight: big ? 600 : 400, color: teal ? tokens.teal : tokens.ink, wordBreak: 'break-word' }}>{v}</Typography>
    </Box>
  );
}
...
export default function KernelDetail() {
  ...
  return (
    <Paper sx={{ borderRadius: 2, overflow: 'hidden', background: tokens.tile2 }}>
      <Stack direction="row" alignItems="center" flexWrap="wrap" useFlexGap sx={{ gap: 1.5, p: '15px 18px', borderBottom: `1px solid ${tokens.hair}`, background: 'linear-gradient(180deg, rgba(31,111,107,.06), transparent)' }}>
        <Typography sx={{ fontFamily: tokens.serif, fontWeight: 600, fontSize: 22, letterSpacing: '-.015em' }}>{s.name.split('.').pop()}</Typography>
