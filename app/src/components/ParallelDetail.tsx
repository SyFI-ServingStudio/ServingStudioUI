...
function Item({ k, v, big, teal, terra }: { k: string; v: string; big?: boolean; teal?: boolean; terra?: boolean }) {
  return (
    <Box sx={{ p: '12px 18px', borderTop: `1px solid ${tokens.hair}`, borderRight: `1px solid ${tokens.hair}`, '&:last-of-type': { borderRight: 'none' } }}>
      <Typography sx={{ fontFamily: tokens.mono, fontSize: 9.5, letterSpacing: '.14em', textTransform: 'uppercase', color: tokens.sub, mb: 0.6 }}>{k}</Typography>
      <Typography sx={{ fontFamily: big ? tokens.serif : tokens.mono, fontSize: big ? 22 : 12.5, fontWeight: big ? 600 : 400, color: terra ? tokens.terra : teal ? tokens.teal : tokens.ink, wordBreak: 'break-word' }}>{v}</Typography>
    </Box>
  );
}
...
function LaneStrip({ imb }: { imb: Imbalance }) {
  ...
  return (
    <Box sx={{ p: '12px 18px 14px', borderTop: `1px solid ${tokens.hair}` }}>
      <Typography sx={{ fontFamily: tokens.mono, fontSize: 9.5, letterSpacing: '.14em', textTransform: 'uppercase', color: tokens.sub, mb: 0.8 }}>
      ...
      <Box sx={{ display: 'flex', alignItems: 'flex-end', gap: many ? '2px' : '4px', height: 46 }}>
        {imb.perLaneAvg.map((v, i) => {
          ...
          return (
            <Tooltip key={i} title={`${imb.laneLabels[i]} · ${v.toFixed(2)}× mean${isStrag ? ' · straggler' : ''}`} arrow enterDelay={80}>
              <Box sx={{ flex: 1, minWidth: 0, height: `${Math.max(6, (v / max) * 100)}%`, borderRadius: '2px 2px 0 0', background: isStrag ? tokens.terra : 'rgba(122,92,255,.45)', outline: isStrag ? `1px solid ${tokens.terra}` : 'none' }} />
            </Tooltip>
          );
        })}
      </Box>
      <Stack direction="row" justifyContent="space-between" sx={{ mt: 0.5, fontFamily: tokens.mono, fontSize: 9, color: tokens.sub2 }}>
        <span>lane 0</span>
        <span>1.0× = balanced · <b style={{ color: tokens.terra }}>straggler {imb.laneLabels[imb.stragglerLane]}</b></span>
...
export default function ParallelDetail() {
  ...
  return (
    <Paper sx={{ borderRadius: 2, overflow: 'hidden', background: tokens.tile2 }}>
      <Stack direction="row" alignItems="center" flexWrap="wrap" useFlexGap sx={{ gap: 1.5, p: '15px 18px', borderBottom: `1px solid ${tokens.hair}`, background: 'linear-gradient(180deg, rgba(122,92,255,.07), transparent)' }}>
        <Typography sx={{ fontFamily: tokens.serif, fontWeight: 600, fontSize: 22, letterSpacing: '-.015em' }}>
          parallel <Box component="span" sx={{ fontFamily: tokens.mono, fontSize: 13, color: tokens.violet }}>⇉ {node.label ?? 'max'}</Box>
        </Typography>
      ...
      <Box sx={{ display: 'grid', gridTemplateColumns: { xs: 'repeat(2,1fr)', md: 'repeat(4,1fr)' } }}>
        ...
        <Item k="straggler lane" v={`${imb.laneLabels[imb.stragglerLane]} · ${imb.stragglerFactor}× mean`} big terra />
