 ...
 * `/api/agent/v1/file*` routes.
 *
 * Owns URL construction and transport errors so panels never build an
 * `/api/agent/v1/file` query string themselves. The backend classifies each path
...
export function fileContentUrl(workspaceId: string, path: string): string {
  return `/api/agent/v1/file?${fileQuery(workspaceId, path)}`;
}
...
export async function getFileMeta(workspaceId: string, path: string): Promise<WorkspaceFileMeta> {
  const response = await requireOk(
    await fetch(`/api/agent/v1/file/meta?${fileQuery(workspaceId, path)}`),
    'Load file metadata',
  );
...
export async function listDirectory(
  ...
  const response = await requireOk(
    await fetch(`/api/agent/v1/file/list?${fileQuery(workspaceId, path)}`),
    'List directory',
  );
