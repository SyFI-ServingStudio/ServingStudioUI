...
export interface KernelPerf {
  tflops: number; gbps: number;
  peakTflops: number; peakGbps: number;
  computeUtil: number; memUtil: number;
...
export function kernelPerf(node: CostNode): KernelPerf {
  ...
  let tf = 0, bw = 0;
  if (g === 'gemm') { tf = H200_PEAK.tflops * (0.55 + 0.16 * r); bw = H200_PEAK.gbps * (0.22 + 0.10 * r); }
  else if (g === 'attn') { tf = H200_PEAK.tflops * (0.30 + 0.15 * r); bw = H200_PEAK.gbps * (0.34 + 0.16 * r); }
  else if (g === 'comm') { tf = H200_PEAK.tflops * (0.005 + 0.010 * r); bw = H200_PEAK.gbps * (0.55 + 0.22 * r); }
  else if (g === 'route') { tf = H200_PEAK.tflops * (0.04 + 0.05 * r); bw = H200_PEAK.gbps * (0.28 + 0.16 * r); }
  else { tf = H200_PEAK.tflops * (0.02 + 0.02 * r); bw = H200_PEAK.gbps * (0.42 + 0.22 * r); } // norm / elementwise
  const computeUtil = tf / H200_PEAK.tflops;
  const memUtil = bw / H200_PEAK.gbps;
  const intensity = (tf * 1e12) / (bw * 1e9); // FLOP per byte
  return {
    tflops: +tf.toFixed(1), gbps: +bw.toFixed(0),
    peakTflops: H200_PEAK.tflops, peakGbps: H200_PEAK.gbps,
    computeUtil: +computeUtil.toFixed(3), memUtil: +memUtil.toFixed(3),
...
export interface DistPoint { x: number; y: number; backend: number; count: number; }
...
export function inputDist(node: CostNode): InputDist {
  ...
  for (let i = 0; i < N; i++) {
    ...
    pts.push({ x: +x.toFixed(3), y: +y.toFixed(3), backend: b, count: 1 + Math.floor(hsh(i + node.id * 3) * 40) });
  }
