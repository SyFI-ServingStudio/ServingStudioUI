...
import type { CostNode, LeafNode, ScaleNode, SumNode } from '../../domain/cost-tree';
import type { OptimalityKernelLadderData } from '../../domain/optimality';
import {
  componentByLeafName,
  filterLadderKernels,
...
describe('normalizeLadderPerCall', () => {
  it('rescales rungs per row and removes scope-wide quantities', () => {
    ...
    const qNorm = normalized.kernels.find(
      (kernel) => kernel.name === 'unified.qk_norm.q_norm',
    )!;
    ...
    const embedding = normalized.kernels.find(
      (kernel) => kernel.name === 'unified.embedding',
    )!;
    expect(embedding.rungs.hardwareLimit).toBe(4);
    // Null necessary attribution stays null instead of becoming 0/divisor.
    const kNorm = normalized.kernels.find(
      (kernel) => kernel.name === 'unified.qk_norm.k_norm',
    )!;
