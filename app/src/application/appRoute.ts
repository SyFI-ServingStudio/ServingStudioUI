...
export function appViewFromHash(hash: string): AppView {
  ...
  if (route === '#/alignment') return 'alignment';
...
/** An alignment bundle — a measured capture beside the prediction of the same
 * shapes — is likewise addressed on its own, not through a run. */
export function alignmentIdFromHash(hash: string): string | null {
  return resourceIdFromHash(hash, 'alignment', 'alignment', /^al_[a-z0-9_]{1,64}$/);
}
...
export function kernelMeasurementIdFromHash(hash: string): string | null {
  return resourceIdFromHash(hash, 'kernel-measurement', 'measurement', /^km_[a-z0-9_]{1,96}$/);
}
